...
export function calculateCodeHealth(health: Health[], linesCount: number, filesCount: number): CodeHealthResponse {
  ...
  output.push(`Found ${BG_ERR}${Intl.NumberFormat('en-US').format(errors)} errors${BG_RESET}, and ${BG_WARN}${Intl.NumberFormat('en-US').format(warnings)} warnings${BG_RESET}, ${BG_INFO}${Intl.NumberFormat('en-US').format(linesCount)} lines${BG_RESET} of code in ${BG_INFO}${Intl.NumberFormat('en-US').format(filesCount)} files${BG_RESET}`)
  ...
  if (codeHealth < LOW_HEALTH_THRESHOLD) {
    ...
    output.push(`${BG_ERR}Code health is LOW: ${codeHealth}%${BG_RESET}`)
  }
  if (codeHealth >= LOW_HEALTH_THRESHOLD && codeHealth < MEDIUM_HEALTH_THRESHOLD) {
    ...
    output.push(`${BG_WARN}Code health is MEDIUM ${codeHealth}%${BG_RESET}`)
  }
  if (codeHealth >= MEDIUM_HEALTH_THRESHOLD && codeHealth < OK_HEALTH_THRESHOLD) {
    ...
    output.push(`${BG_INFO}Code health is OK: ${codeHealth}%${BG_RESET}`)
  }
  if (codeHealth >= OK_HEALTH_THRESHOLD) {
    //console.log(`${BG_OK}Code health is GOOD: ${codeHealth}%${BG_RESET}`)
    output.push(`${BG_OK}Code health is GOOD: ${codeHealth}%${BG_RESET}`)
  }
